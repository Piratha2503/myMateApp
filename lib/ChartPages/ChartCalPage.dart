...
class _ChartCalPageState extends State<ChartCalPage>{
  ...
  Future<List<String>> _fetchCityNames(String query) async {
    final url = "http://192.168.1.55:8092/city/findcities?cityName=$query&countryCode=lk";
  ...
  void onPressed() async{
    print("Hi");
    const url = "http://192.168.1.55:8092/city/findcities?cityName=gall&countryCode=lk";
    final uri = Uri.parse(url);
    final response = await http.get(uri);
